...
public class SegmentTree {
    ...
    */
    ...
    public void updateRangeQuery(int index, int val) {
        arr[index] = val;
        updateRangeQuery(0, arr.length - 1, 0, index, val);
    }

    private void updateRangeQuery(int low, int high, int segmentIndex, int index, int val) {
        if (low == high) {
        ...
        if (index <= mid) {
            updateRangeQuery(low, mid, 2 * segmentIndex + 1, index, val);
        } else {
            updateRangeQuery(mid + 1, high, 2 * segmentIndex + 2, index, val);
        }
